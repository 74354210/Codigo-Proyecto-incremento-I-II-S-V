<?php 
 ...
 $conexion = new PDO('mysql:host=localhost;dbname=hoja_azul','root','');
	...
	if (isset($_POST['submit'])) {
		...
		$sql = $conexion->prepare("SELECT * FROM docentes WHERE dni LIKE :dni OR apellido LIKE :apellido");
		$resultado = $sql->execute(array(':dni'=> "%$buscar%",
			                              ':apellido'=>"%$buscar%"));
		$resultado = $sql->fetchAll(PDO::FETCH_OBJ);
	if (empty($resultado)) {
		$titulo = 'No se encontraron Docentes  : ' .$buscar;
	} else {
		$titulo = 'El Docente Buscado es : ' . $buscar;
	}
...
<tbody>
	...
	<td>&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;&nbsp; &nbsp;
		<a href="listadoDocentes"><i class="fa fa-edit btn btn-primary "></i>
		</a>
		<a href="alta&iddocente=<?php echo $resultados->iddocente; ?>"><i class="fa fa-edit btn btn-warning "></i>
		</a>
   </td>
</tbody>
<?php endforeach; ?>
